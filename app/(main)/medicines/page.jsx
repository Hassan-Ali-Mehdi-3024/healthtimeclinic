  ...
  const fetchData = async () => {
    ...
    try {
      const [invRes, comboRes] = await Promise.all([
        fetch('/api/medicines?type=products'),
        fetch('/api/medicines?type=combination')
      ]);
      const invData = await invRes.json();
      const comboData = await comboRes.json();
      
      // Strict filter for Inventory to ONLY show base medicines
      const validBaseMedicines = ['COBECWT', 'COBECWRT', 'COBECGT', 'COBECYT', 'COBECPT', 'SLIM-X'];
      const filteredInvData = invData.filter(item => validBaseMedicines.includes(item.name));
      
      setInventory(filteredInvData);
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ 
        ...
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: '700', color: 'var(--text-primary)', margin: 0 }}>Medicines & Inventory</h1>
          <p style={{ color: 'var(--text-secondary)', marginTop: '0.5rem' }}>Manage all medicines, stock batches, and combinations.</p>
        </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
            <div style={{ position: 'relative', flex: '1', minWidth: '250px' }}>
              ...
              <input
                type="text"
                placeholder="Search by name, batch #, DRAP ACT #..."
                value={searchQuery}
          ...
          <div style={{ backgroundColor: isMobile ? 'transparent' : 'white', borderRadius: isMobile ? '0' : '12px', boxShadow: isMobile ? 'none' : 'var(--shadow-sm)', overflow: 'hidden', border: isMobile ? 'none' : '1px solid #e2e8f0' }}>
            {isMobile ? (
              // Mobile Card View
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {loading ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-secondary)' }}>No inventory batches found.</div>
                ) : (
                  filteredInventory.map(item => {
                    const expiryStatus = getExpiryStatus(item.next_expiry);
                    const stockStatus = getStockStatus(item.total_stock);
                      ...
                      >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                          <div>
                            ...
                            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{item.batch_count} Batches</div>
                          </div>
                        </div>

                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                          <span style={{ fontSize: '0.75rem', padding: '0.25rem 0.5rem', borderRadius: '4px', backgroundColor: stockBadge.bg, color: stockBadge.color, fontWeight: '500' }}>
                            {stockBadge.text} ({item.total_stock})
                          </span>
                          {expiryBadge && (
                            <span style={{ fontSize: '0.75rem', padding: '0.25rem 0.5rem', borderRadius: '4px', backgroundColor: expiryBadge.bg, color: expiryBadge.color, fontWeight: '500' }}>
                              Next Expiry: {item.next_expiry ? new Date(item.next_expiry).toLocaleDateString() : 'N/A'}
                            </span>
                          )}
                        </div>
                      </div>
              ...
              <thead style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                <tr>
                  ...
                  <th style={{ padding: '1rem', fontWeight: '600' }}>Total Stock</th>
                  <th style={{ padding: '1rem', fontWeight: '600' }}>Batches</th>
                  <th style={{ padding: '1rem', fontWeight: '600' }}>Next Expiry</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan="5" style={{ padding: '2rem', textAlign: 'center' }}>No medicines found.</td></tr>
                ) : (
                  filteredInventory.map(item => {
                    const expiryStatus = getExpiryStatus(item.next_expiry);
                    const stockStatus = getStockStatus(item.total_stock);
                    ...
                    return (
                      <tr key={item.id} style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }} onClick={() => router.push(`/medicines/${item.id}`)}>
                        ...
                        <td style={{ padding: '1rem' }}>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                            <span>{item.total_stock} boxes</span>
                        ...
                        <td style={{ padding: '1rem' }}>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                            <span>{item.next_expiry ? new Date(item.next_expiry).toLocaleDateString() : 'N/A'}</span>
                            {expiryBadge && (
                        ...
                        <td style={{ padding: '1rem' }}>
                          <button style={{ color: 'var(--primary-color)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: '500' }}>View Batches</button>
                        </td>
                      </tr>
                    );
                  })
      ...
      {showComboForm && (
        <CombinationForm
          ...
          availableMedicines={['COBECWT', 'COBECWRT', 'COBECGT', 'COBECYT', 'COBECPT', 'SLIM-X']}
