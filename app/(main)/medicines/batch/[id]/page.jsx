...
const BatchDetail = () => {
  ...
  const fetchItem = async () => {
    try {
      const response = await fetch(`/api/medicines/${id}?type=batch`);
  ...
  if (!item) return <div style={{ padding: '2rem' }}>Batch not found.</div>;
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          <button onClick={() => router.push(`/medicines/batch/${id}/edit`)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', backgroundColor: 'white', color: 'var(--text-primary)', border: '1px solid #e2e8f0', borderRadius: '8px', fontWeight: '500', cursor: 'pointer' }}>
